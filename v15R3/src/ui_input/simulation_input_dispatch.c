...
void simulation_input_dispatch(GtkWidget *parent_window) {
    ...
    /* FTC transplant: gamepad drive (poll once per frame when robots exist). */
    if (gui_robot_get_count() > 0) {
        gamepad_poll(gamepad_get_primary());
    }
    if (gui_robot_get_count() > 0) {
        float drive_forward = 0.0f, drive_strafe = 0.0f, drive_rotate = 0.0f;
        gamepad_state *pad = gamepad_get_primary();
        if (gamepad_is_connected(pad)) {
            drive_forward = gamepad_get_axis(pad, gamepad_axis_left_y);
            drive_strafe = gamepad_get_axis(pad, gamepad_axis_left_x);
            drive_rotate = gamepad_get_axis(pad, gamepad_axis_right_x);
