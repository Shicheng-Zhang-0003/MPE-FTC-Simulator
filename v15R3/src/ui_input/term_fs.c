...
void cmd_touch(int argc, char **argv) {
    ...
    if ((argc > 1) && (strstr(argv[1], "robot"))) {
        ...
        int robot_index = gui_robot_spawn(0.0f, ftc_robot_rest_height(), 0.0f, MOTOR_GB_5203_19_2);
        if (robot_index < 0) {
            term_err("mpe: touch robot: spawn failed (max robots, no world, or no capacity)\n");
            return;
        }
        term_printf("term_ok", "/robot/%d created at (0.0, %.2f, 0.0) motor=GB5203-19.2:1\n",
