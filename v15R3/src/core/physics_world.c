...
static void physics_world_process_pair(physics_world *world, int index_a, int index_b, float dt,
    ...
    /* FTC transplant: skip contact between joint-connected bodies. */
    if (constraint_bodies_connected(world, body_a->object_id, body_b->object_id)) return;
        ...
        if ((body_a->type == object_sphere) && (body_b->type == object_sphere)) {
            ...
            if ((*manifold_count_ptr) >= a3_max_manifolds) {
                ...
                /* TRUTH: dropped pair still touches: flag support so the
                 * bodies never take the free-flight gravity path this tick. */
                if (world->has_contact) {
                    if ((index_a >= 0) && (index_a < world->body_count)) {
            ...
            if ((a_was_sleeping) && (b_was_sleeping)) {
                ...
                (*manifold_count_ptr)++;
                /* Support flag for gravity-exactness gating: set whenever a
                 * contact manifold exists. Slop-band near-misses solve to
                 * zero normal impulse (clamped >= 0), so they neither hold
                 * weight nor drag via friction; gating the flag on
                 * penetration > 0 instead misclassifies exact touch (pen 0)
                 * as flight and pumps hover-touch limit cycles. Presence is
                 * the honest signal. */
                if (world->has_contact) {
                    if ((index_a >= 0) && (index_a < world->body_count)) {
                        world->has_contact[index_a] = 1;
...
static void physics_world_step_single(physics_world *world, float dt);

void physics_world_step(physics_world *world, float dt) {
    if ((!world) || (!world->bodies) || (!(dt > 0.0f)) || (!isfinite(dt)) || (world->body_count <= 0)) {
        return;
    }
    /* TRUTH: big steps hide stiffness (error ~ dt^2) and fake stability.
     * Substep dt > 1/60 into ceil(dt*60) equal pieces instead of silently
     * clamping away simulated time. Headless 1/60 ticks take n=1: zero
     * behavior change for the suite. */
    const float base_step = 1.0f / 60.0f;
    if (dt > base_step * (1.0f + 1e-6f)) {
        int n = (int) ceilf(dt / base_step);
        if (n < 1) {
            n = 1;
        }
        if (n > 32) {
        ...
        float sub = dt / (float) n;
        for (int k = 0; k < n; k++) {
            physics_world_step_single(world, sub);
        }
        return;
    }
    physics_world_step_single(world, dt);
}
...
static void physics_world_step_single(physics_world *world, float dt) {
    ...
    for (int i = 0; i < world->body_count; i++) {
        ...
        if (collision_static_plane_body(rb, 0.0f, &floor_collision)) {
            ...
            if (manifold_count >= a3_max_manifolds) {
                world->manifold_overflow_count++;
                /* TRUTH: dropped floor contact still supports. */
                if (world->has_contact) {
            ...
            manifold_count++;
            /* Support flag on manifold presence (see pair path). */
            if (world->has_contact) {
                world->has_contact[i] = 1;
            }
        }
    ...
    /* NOTE (measured, F10): no wake-on-separation here. Waking sleepers
     * merely for lacking a manifold this tick reintroduces the never-sleep
     * micro-motion pump (10-stack ejects at ~13 m/s): settling bodies
     * separate briefly and must keep their sleep. Sticky sleep + the
     * three-gate wake (velocity/depth/novelty) below own wakefulness.
     * Corner accepted: a sleeper whose support is DELETED floats until a
     * real gate (or explicit wake) fires; terminal users own that state. */
    ...
    /* TRUTH: gravity as direct velocity change (v += g*dt). Routing m*g
     * through inverse_mass adds 1ulp rounding ((m*g)/m != g bit-exact).
     * Other forces still use the accumulator path. */
    for (int i = 0; i < world->body_count; i++) {
        rigidbody *rb = &world->bodies[i];
        if ((rb->static_state) || (rb->is_sleeping) || (rb->kinematic)) {
            continue;
        }
        rb->velocity.y += step_gravity * dt;
    }
    ...
    /* TRUTH: integrate CCD remainder; gravity-exactness for contact-free
     * bodies ONLY (x += v*rem - 1/2*g*rem^2 is the exact constant-force
     * flow, hence exact parabolas; constrained bodies stay symplectic Euler
     * since contact impulses cancel gravity post-solve). Gated on vacuum
    ...
    for (int i = 0; i < world->body_count; i++) {
        ...
        if (vacuum_exact && free_flight && (!ib->static_state) && (!ib->is_sleeping) && (!ib->kinematic) &&
            (step_dt > 0.0f)) {
