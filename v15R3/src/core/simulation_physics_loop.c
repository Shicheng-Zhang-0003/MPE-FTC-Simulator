...
void simulation_physics_tick(float frame_delta_time) {
    ...
    while (physics_time_accumulator >= fixed_physics_dt) {
        ...
        /* NOTE: no wake-on-separation (see world path; F10-measured). */
        ...
        /* TRUTH: forces after narrow (unified with world path). Gravity is
         * a direct velocity change (exact, no 1ulp m*g/m rounding). */
        ...
        for (int gi = 0; gi < world->body_count; gi++) {
            ...
            rb->velocity.y += g_cfg.world.gravity * fixed_physics_dt;
        }
         ...
         * TRUTH: CCD remainder + gravity-exact free flight in vacuum only. */
        bool a3_boundary_moved_any = false;
        float grav_half_leg = -0.5f * g_cfg.world.gravity;
        bool vacuum_exact_leg = (g_cfg.world.drag == 1.0f);
        for (int object_iterator_index = 0; object_iterator_index < world->body_count; object_iterator_index++) {
            ...
            if (vacuum_exact_leg && free_flight && (!rigid_body->static_state) && (!rigid_body->is_sleeping) &&
