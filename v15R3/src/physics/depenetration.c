...
void a3_positional_depenetration_pass_dt(struct physics_world *world, broadphase_pair *pair_buffer,
    ...
    for (int dep_iteration = 0; dep_iteration < depenetration_iterations; dep_iteration++) {
        for (int pair_index = 0; pair_index < pair_count; pair_index++) {
            ...
            /* TRUTH: never positionally separate joint-connected bodies.
             * The joint velocity solve owns that DOF; teleporting here then
             * re-solving there pumps energy every tick (hinge jitter). */
            if (constraint_bodies_connected(world, body_a->object_id, body_b->object_id)) {
                continue;
            }
