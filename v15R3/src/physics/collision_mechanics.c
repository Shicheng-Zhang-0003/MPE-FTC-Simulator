...
void collision_prepare_solver(struct physics_world *world, collision_data *source, collision_data *m, float dt) {
    *m = *source;
    ...
    for (int i = 0; i < m->contact_count; i++) {
        ...
        /* TRUTH: warm friction direction memory for the projection below. */
        vector3 warm_old_dir = vector3_zero();
        int warm_have_old = 0;
        ...
        if ((hash_head) && (cache_id_a != 0) && (cache_id_b != 0)) {
            ...
            for (int32_t slot = hash_head[slot0], guard = 0;
                ...
                if (role == 1) {
                    ...
                    cp->accumulated_normal_impulse = fmaxf(cc->accumulated_normal_impulse, 0.0f);
                    cp->accumulated_tangent_impulse = -cc->accumulated_tangent_impulse;
                    warm_old_dir = vector3_scaling(cc->tangent_dir, -1.0f);
                    warm_have_old = 1;
                    cache_match_found = 1;
                    break;
                }
        ...
        /* TRUTH: separating pairs are not persistent contact. A cached
         * impulse belongs to an ongoing compression/resting state; when
         * the pair opens (vn > 0), re-applying stale normal impulse pushes
         * opening bodies apart and stale friction points along dead slip —
         * both pump settling stacks (cold solve settles calm, warm jiggles:
         * measured). Drop restored history on separation; resting and
         * approaching contacts keep full warm start (static hold intact).
         * Poisson still sees this tick's true compression delta via
         * base_normal_impulse below (fresh impacts bounce correctly). */
        if (vn_initial > 1e-4f) {
            cp->accumulated_normal_impulse = 0.0f;
            cp->accumulated_tangent_impulse = 0.0f;
        }
        ...
        {
            rigidbody *mw = NULL;
            if (m->object_a && m->object_a->is_mecanum) mw = m->object_a;
        ...
        /* TRUTH: warm friction is magnitude-only memory. The restored
         * impulse was accumulated against LAST tick's slip frame; if slip
         * reversed (or the frame rotated), re-applying the full magnitude
         * injects up to mu*N along the wrong direction every tick, pumping
         * settling stacks (never-sleep micro-motion). Project onto the
         * current frame: persistent direction keeps its magnitude (static
         * hold unaffected), reversal zeroes it. Normal is frame-free. */
        if ((warm_have_old) && (cp->accumulated_tangent_impulse != 0.0f) &&
            (vector3_length_squared(cp->tangent_vector) > 0.0001f) &&
            (vector3_length_squared(warm_old_dir) > 0.0001f)) {
            vector3 old_n = vector3_scaling(warm_old_dir,
                1.0f / sqrtf(vector3_length_squared(warm_old_dir)));
            float keep = vector3_dot(old_n, cp->tangent_vector);
            if (keep < 0.0f) {
                keep = 0.0f;
            } else if (keep > 1.0f) {
                keep = 1.0f;
            }
            cp->accumulated_tangent_impulse *= keep;
        }
...
float collision_resolve_iterative(collision_data *m, float dt, bool friction_only, int start_index) {
    ...
    for (int k = 0; k < m->contact_count; k++) {
        ...
        if (vector3_length_squared(tangent) > 0.0001f) {
             ...
             * Anisotropic (mecanum rollers): DECOUPLED independent clamps.
             * Grip = rubber Coulomb, free = roller-bearing drag: separate
             * physical limits. A coupled radial/elliptical scale forces
             * a_grip ~= a_free and cancels mecanum lateral thrust. */
            ...
            if (cp->anisotropic && has_t2 && cp->mu_grip > 0.0f && cp->mu_free > 0.0f &&
                cp->accumulated_normal_impulse > 0.0f) {
                /* Stick/slip select on the grip axis, mirroring isotropic. */
                float mu_g = (slip_speed < static_friction_threshold)
                    ? fmaxf(cp->mu_grip, kinetic_friction_coeff)
                    : kinetic_friction_coeff;
                float max_g = cp->accumulated_normal_impulse * mu_g;
                float max_f = cp->accumulated_normal_impulse * cp->mu_free;
                if (new_acc1 > max_g) new_acc1 = max_g;
                else if (new_acc1 < -max_g) new_acc1 = -max_g;
                if (new_acc2 > max_f) new_acc2 = max_f;
                else if (new_acc2 < -max_f) new_acc2 = -max_f;
            } else {
        ...
        for (int i = 0; i < man->contact_count; i++) {
            ...
            }
            {
                ...
                for (int bi = 0; bi < 2; bi++) {
                     ...
                     * lever = Hertz patch a=sqrt(R*pen), capped 0.3R.
                     ...
                     * R, spin resists yaw via patch. Zero measured depth
                     * means no patch: the spin term skips (no invented
                     * torque); the roll term needs only contact. */
                    float pen_raw = (cp->penetration > 0.0f) ? cp->penetration : 0.0f;
                    /* TRUTH: Hertz patch needs real indentation. The old
                     * 0.5mm floor fabricated spin torque from zero-depth
                     * slop support. No depth -> no patch -> no spin term
                     * (roll term below needs only contact, not depth). */
                    float pen_eff = pen_raw;
...
int collision_ccd_sweep_clamp_full(rigidbody *bodies, int body_count, float dt, float *time_remaining_out) {
    ...
    if (record_remainder) {
        for (int i = 0; i < body_count; i++) {
            ...
            /* TRUTH: gravity acts during the TOI segment too. Without the
             * 1/2*g*toi^2 term the pre-move drops up to 1.36mm (dt=0.1
             * clamp) that the remainder pass never restores. Springs/motors
             * during toi are second-order (toi << dt for fast bodies). */
            mover->position.y += 0.5f * g_cfg.world.gravity * toi * toi;
