...
bool collision_static_plane_cylinder(rigidbody *cyl, float plane_y, collision_data *collision_output_data) {
    ...
    if (collision_output_data->contact_count == 0) {
        ...
        /* TRUTH: exact leg constants, never libm trig. cosf/sinf vary at
         * ~1e-7 across libm implementations (desync); these four legs are
         * exactly (1,0),(0,1),(-1,0),(0,-1). */
        static const float leg_c[4] = {1.0f, 0.0f, -1.0f, 0.0f};
        static const float leg_s[4] = {0.0f, 1.0f, 0.0f, -1.0f};
...
bool collision_cylinder_cube(rigidbody *cyl, rigidbody *cube,
     ...
     * Exact for face-parallel (the wheel-lying case); no-op otherwise. */
    {
        ...
        if (axis_face < 0.1f) {
            ...
            /* TRUTH: exact leg constants (0/120/240 deg), never libm trig.
             * cos120=cos240=-1/2 exactly; sin=±sqrt(3)/2 (sqrtf is IEEE
             * exact, bit-identical everywhere). */
            static const float rim_c[3] = {1.0f, -0.5f, -0.5f};
            static const float rim_s[3] = {0.0f, 0.8660254f, -0.8660254f};
            for (int leg = 0; leg < 3; leg++) {
                ...
                vector3 rim = vector3_addition(
                    cap_center,
                    vector3_scaling(vector3_addition(vector3_scaling(u, rim_c[leg]),
                                                    vector3_scaling(v, rim_s[leg])),
                                    r));
