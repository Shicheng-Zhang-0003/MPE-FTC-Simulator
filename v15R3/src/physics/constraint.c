...
/* FTC transplant: jointed-body self-collision filter. Suppresses contact
 * only for RIGID joints (revolute hinges, fixed welds, prismatic sliders)
 * whose solver owns the shared DOF. Distance rods and ropes MUST still
 * collide: a slack rope / resting rod pair interpenetrating with zero
 * contact impulse is the wrong answer (pass-through). */
bool constraint_bodies_connected(const struct physics_world *world, uint32_t id_a, uint32_t id_b) {
    if (!world || id_a == 0 || id_b == 0 || id_a == id_b) return false;
    for (int i = 0; i < mpe_max_joints; i++) {
        if (!world->revolute_constraints[i].is_active) continue;
        constraint_type t = world->revolute_constraints[i].type;
        if ((t != constraint_revolute) && (t != constraint_fixed) && (t != constraint_prismatic)) {
            continue;
        }
        uint32_t a = world->revolute_constraints[i].body_id_a;
        uint32_t b = world->revolute_constraints[i].body_id_b;
        if ((a == id_a && b == id_b) || (a == id_b && b == id_a)) return true;
    }
    return false;
}
...
static void constraint_dispatch (struct physics_world *world, float dt, bool motors_pass) {
    ...
    for (int i = 0; i < mpe_max_joints; i++) {
         ...
         * its bodies awake: motor torque is written straight to the
         * accumulator (no force-wake trip), so a sleeping robot would
         * otherwise never hear its own drive. Zero-target motors hold
         * without vetoing sleep. */
        if (motors_pass) {
            if ((c->type == constraint_revolute) && (c->p.revolute.motor_enabled) &&
                (isfinite (c->p.revolute.motor_target_speed)) && (fabsf (c->p.revolute.motor_target_speed) > 1e-6f)) {
                rigidbody_wake (body_a);
                rigidbody_wake (body_b);
            }
            if ((c->type == constraint_prismatic) && (c->p.prismatic.motor_enabled) &&
                (isfinite (c->p.prismatic.motor_target_speed)) &&
                (fabsf (c->p.prismatic.motor_target_speed) > 1e-6f)) {
                rigidbody_wake (body_a);
                rigidbody_wake (body_b);
            }
        } else {
