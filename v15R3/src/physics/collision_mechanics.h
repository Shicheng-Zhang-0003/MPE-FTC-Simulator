...
typedef struct {
    ...
    /* FTC mecanum anisotropic (transplant): when true, tangent=grip axis
     * (mu_grip) and tangent2=roller free axis (mu_free), clamped elliptically
     * (a1/mu_g)^2+(a2/mu_f)^2<=N^2 instead of circular. False = isotropic. */
    bool anisotropic;
    float mu_grip;
    float mu_free;
