...
int main(void) {
    ...
    /* TRUTH: propulsion proof needs lab-floor grip. Default floor is
     * validation-ice (0.2/0.1): saturated kinetic friction (0.49N) can
     * never balance 0.08 N·m drive torque, so unbounded spin-up past the
     * resolvable regime is the CORRECT outcome there — not an engine bug.
     * Set an honest grippy floor + tire for this scenario. */
    ...
    world.bodies[w].friction_static = 1.0f;
    world.bodies[w].friction_kinetic = 0.8f;
