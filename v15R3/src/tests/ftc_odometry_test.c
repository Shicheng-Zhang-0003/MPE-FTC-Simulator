...
int main(void) {
    ...
    /* TRUTH: encoder odometry on foam under full-power drive slips 10-20%
     * (wheels out-spin travel; measured 9.6-18.6% across honest configs —
     * a point gate below that overfits solver details, not hardware).
     * Gate at 20% (was 30%, legalizing 88% slip fantasies); direction and
     * strafe-sign checks below carry the fidelity proof. */
    if (dist_error > 0.20f) {
