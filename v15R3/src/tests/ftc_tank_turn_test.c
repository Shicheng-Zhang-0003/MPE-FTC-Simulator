...
int main(void) {
    ...
    /* Robot should have rotated but not translated much. TRUTH: yaw from
     * the quaternion (2*atan2(qy,qw)), not the raw qy component (which only
     * approximates yaw/2 at small angles). */
    float displacement = sqrtf((end_x - start_x) * (end_x - start_x) +
                               (end_z - start_z) * (end_z - start_z));
    vector4 qh = world.bodies[robot.chassis_body].orientation;
    float heading_change = fabsf(2.0f * atan2f(qh.y, qh.w));
