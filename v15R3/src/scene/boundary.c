...
static int g_boundary_trips = 0;
int boundary_trip_count(void) {
    return g_boundary_trips;
}
...
void boundary_apply_floor(rigidbody *rigid_body, float floor_y_level) {
    ...
    bool is_kinematic = rigid_body->kinematic;
    ...
    if (min_y < (floor_y_level - g_cfg.boundary.floor_emergency_slop)) {
        ...
        if ((!is_kinematic) && (rigid_body->velocity.y < 0.0f)) {
...
void boundary_apply_box(rigidbody *rigid_body, vector3 min_bounds, vector3 max_bounds) {
    ...
    /* TRUTH: kinematic platforms get positionally clamped like everyone
     * else (a prescribed-velocity platform must not leave the playable
     * volume forever); only their prescribed VELOCITY is preserved (never
     * zeroed below). Old code returned early for kinematics. */
    bool is_kinematic = rigid_body->kinematic;
    if (rigid_body->static_state) {
        return;
    }
    ...
    if (min_x < min_bounds.x) {
        ...
        rigidbody_wake(rigid_body);
        g_boundary_trips++;
        if ((!is_kinematic) && (rigid_body->velocity.x < 0)) {
    ...
    if (max_y > max_bounds.y) {
        ...
        rigidbody_wake(rigid_body);
        g_boundary_trips++;
        if ((!is_kinematic) && (rigid_body->velocity.y > 0)) {
            rigid_body->velocity.y = 0.0f;
        }
    } // Z axis
    ...
    if (min_z < min_bounds.z) {
        ...
        rigidbody_wake(rigid_body);
        g_boundary_trips++;
        if ((!is_kinematic) && (rigid_body->velocity.z < 0)) {
