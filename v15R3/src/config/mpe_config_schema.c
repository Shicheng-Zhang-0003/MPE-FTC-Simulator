...
static mpe_param s_registry[] = {
    ...
    {"world.drag", "Air Drag Coefficient", "VISCOUS retention base (linear viscous c=-ln(drag), NOT quadratic aero; area/mass-independent; 1.0=truth vacuum, off)", p_float,
...
{"timestep.solver_iterations", "Solver Iterations",
    ...
    {"timestep.max_linear_speed", "Max Linear Speed", "INFORM overflow counter only (validation_report); never scales velocities (no guillotine: CCD+substeps own fast bodies)",
     p_float, cat_timestep,
     &g_cfg.timestep.max_linear_speed, 150.0, 10.0, 10000.0, true},

    {"timestep.max_angular_speed", "Max Angular Speed", "INFORM overflow counter only (validation_report); never scales spin (no guillotine)",
     p_float, cat_timestep,
...
{"solver.wheel_lock_omega_thresh", "Wheel Lock Omega Threshold",
    ...
    {"depenetration.max_correction", "Max Correction", "Per-pass positional correction cap (m; last-resort projection, bounded so deep pathology converges over ticks instead of teleporting)", p_float,
    ...
    {"depenetration.rebuild_iterations", "Rebuild Iterations", "Depenetration iterations after boundary rebuild (bounded with max_correction above)", p_int,
