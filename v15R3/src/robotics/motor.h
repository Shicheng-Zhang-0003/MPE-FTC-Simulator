...
typedef struct {
    ...
    float kt; /* N·m/A torque constant (from stall) */
    float kv; /* V/(rad/s) back-EMF constant (from free speed; Kt/Ke gap = lumped loss) */
    float stall_current; /* A */
    float free_speed_rad_s; /* rad/s at no load */
    float no_load_current; /* A drawn spinning free (brush/gear loss measure) */
