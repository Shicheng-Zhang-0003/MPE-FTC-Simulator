...
void simulation_physics_tick(float frame_delta_time) {
    ...
    float angular_damping_factor = powf(g_cfg.world.drag * g_cfg.world.angular_damping_factor, fixed_physics_dt);
    ...
    while (physics_time_accumulator >= fixed_physics_dt) {
        ...
        for (int collision_index = 0; collision_index < detected_collision_count; collision_index++) {
            ...
            else if (rigid_body_a->type == object_cylinder && rigid_body_b->type == object_sphere)
                collided = collision_cylinder_sphere(rigid_body_a, rigid_body_b, &narrowphase_collision);
            else if (rigid_body_a->type == object_sphere && rigid_body_b->type == object_cylinder) {
                collided = collision_cylinder_sphere(rigid_body_b, rigid_body_a, &narrowphase_collision);
                narrowphase_collision.normal_vector = vector3_scaling(narrowphase_collision.normal_vector, -1.0f);
                narrowphase_collision.object_a = rigid_body_a;
                narrowphase_collision.object_b = rigid_body_b;
            } else if (rigid_body_a->type == object_cylinder && rigid_body_b->type == object_cube)
                collided = collision_cylinder_cube(rigid_body_a, rigid_body_b, &narrowphase_collision);
            else if (rigid_body_a->type == object_cube && rigid_body_b->type == object_cylinder) {
                collided = collision_cylinder_cube(rigid_body_b, rigid_body_a, &narrowphase_collision);
                narrowphase_collision.normal_vector = vector3_scaling(narrowphase_collision.normal_vector, -1.0f);
                narrowphase_collision.object_a = rigid_body_a;
                narrowphase_collision.object_b = rigid_body_b;
            } else if (rigid_body_a->type == object_cylinder && rigid_body_b->type == object_cylinder)
                collided = collision_cylinder_cylinder(rigid_body_a, rigid_body_b, &narrowphase_collision);
            if (collided) {
